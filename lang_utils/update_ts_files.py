...
def fill_missing_translations(ts_file: str, translation_file: str) -> None:
    ...
    ts_hand: ElementTree[Element[str]] = et.parse(ts_file)

    index = 0
    for tag in ts_hand.findall('context'):
        for tag in tag.findall('message'):
            translation: Element[str] | None = tag.find('translation')
    ...
    with open(ts_file, 'wb') as f:
        f.write(et.tostring(ts_hand.getroot(), encoding='utf-8'))
