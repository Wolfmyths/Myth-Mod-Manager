...
def test_thread(qtbot: QtBot, create_mod_dirs: str, createTemp_Config_ini: str, createTemp_Mod_ini: str, mod_name: str, disabled: bool) -> None:  # pyright: ignore[reportUnusedParameter]
    Save.setEnabled(mod_name, disabled)
    mutex = QMutex()
    worker = DeleteMod('make game easy mod')
    worker.mutex = mutex

    with qtbot.waitSignal(worker.succeeded):
        worker.start()
