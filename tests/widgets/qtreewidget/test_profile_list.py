...
def test_modProfile(qtbot: QtBot, create_ProfileList: ProfileList) -> None:
    ...
    profile: QTreeWidgetItem = create_ProfileList.__findProfile('Awesome mods')
    list_of_mods: list[QTreeWidgetItem] | None = create_ProfileList.__getMods(profile)
...
def test_addMods(create_ProfileList: ProfileList) -> None:
    profile: QTreeWidgetItem = create_ProfileList.__findProfile('newprofile')
...
def test_isProfile(create_ProfileList: ProfileList) -> None:
    profile: QTreeWidgetItem = create_ProfileList.__findProfile('newprofile')
    mod: QTreeWidgetItem | Any = create_ProfileList.__getMods(profile)[0]
...
def test_copyModsToProfile(create_ProfileList: ProfileList) -> None:
    profile: QTreeWidgetItem = create_ProfileList.__findProfile('Awesome mods')
    copyToProfile: QTreeWidgetItem = create_ProfileList.__findProfile('editedprofile')
    profile.setSelected(True)
    create_ProfileList.copyModsToProfile('editedprofile')

    profile.setSelected(False)

    mods: list[str] = [x.text(0) for x in create_ProfileList.__getMods(copyToProfile)]
    assert len(mods) == 4
...
def test_deleteProfile(create_ProfileList: ProfileList) -> None:
    ...
    assert 'editedprofile' not in create_ProfileList.__getProfiles()
